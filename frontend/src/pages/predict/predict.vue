  ...
  市场信号: {
    ...
    blocks: [
      ...
      },
      {
        title: '为何只看这几家',
        paras: [
          '只取 Pinnacle、Bet365、皇冠、威廉希尔、伟德。足彩网的沙巴按皇冠计入；韦德是跟盘公司，不进这张票。',
          'Pinnacle 是 Sharp（权重最高），用来校验诱盘是否靠谱；Bet365/皇冠是华语亚盘主市场；威廉、伟德作补充。伟德在足彩网没有，缺票也不用韦德顶。',
        ],
      },
    ],
  },
  市场热度: {
    ...
    blocks: [
      ...
      },
      {
        title: '为何列出这么多家',
        paras: [
          '热度看的是资金共识，和市场上信号只看五家主流不同。足彩网热度池是主流加澳门、立博、韦德。',
...
const reverseTagText = computed(() => {
  ...
  if (p.reverseReason) {
    return `逆向修正：${p.reverseReason}，加权仍跟风，已整体反向 → ${final}`
  }
...
async function startAnalysis() {
  ...
  try {
    ...
    prediction.value = {
      ...
      reverseReason: pred.reverse_reason || null,
    }
